...
a unique and identifier for each table to share data and stuff like that we mostly
use id that has auto increment

## Foreign Key

for example in our example the Student is a parent and enrollment is foreign because we
can't have enrollment without student and we connect both into each other we have key called
student_id that is foreign key because the value is came by student model id.

## Normalization

it basically follow some rules to prevent duplication it has 7 form but for most of applications
you need only 3 of it

### First Normal Form (1NF)

> Each cell should have a **single value** and we cannot have repeated columns.

The point is turn that data into single table and create relationship with others

## Link Table

for example in our design we create another table called course_tag then connect that
to tags and courses

![](./images/link_table.png)

### Second Normal Form (2NF)

> Every **table** should describe **one entity** and every column in that table should describe that entity.

for example in our example the name of instructor because it can teach several course and it name duplicate in
multiply places so we have to put it in another table

### Third Normal Form (3NF)

> A column in a table should not be derived from other columns.

like this example that we can remove full_name because we can calculate that by other columns
...
or we can do like this instead of removing
...
the same thing you set for your html pages utf8 is support all language
for latin it store 1 bitte for middle east 2 bite and for asian language 3

for example if you use VARCHAR(10) and you 1 million record it actually store
30 M but if you specifically for latin language it would be 10 M we can do it like this
...
MyISAM -> is old version
InnoDB -> mostly commonly used and new one
...
![](./images/query_custoemr_table.png)

as default it check all of them

![](./images/create_index_for_customers.png)

we can use indexes to speed up and don't check each one by one and it save in memory
...
by this way we can see how mySQL run this

```sql
EXPLAIN SELECT customer_id FROM customers WHERE state = 'CA';
```

for code it show type **ALL** and it check in 1010 rows
...
now after creating index the type become **ref** amd iot only check 112 rows
...
for more accurate result we add this as well
...
for string varchar and text and stuff like that get so much space we can prefix our
values to just store few of them for performance
...
in our example 5 and 10 dose not get any improvement while we double the space so we go
with 5

## Full-text Indexes

this low of searching
...
- Put the columns with a higher cardinality (unique values) first
...
at this code it check all rows 1010
...
but rare to do this and using always in most situation is not suitable
and you can see the cost of your operation like this
...
A covering index is an index that contains all the columns needed to satisfy
a query, so the database can return results directly from the index without
accessing the actual table. For example, if you often run
`SELECT id, name FROM users WHERE id = 123`, creating
`CREATE INDEX idx_users_covering ON users (id, name)` lets the database
find the row using `id` and immediately get `name` from the index,
avoiding a separate table lookup.

## index Maintenance

![](./images/duplicate_redundant_indexes.png)

> Before creating new indexes , check the existing ones.

## Creating a User

this how to restrict user IP

```sql
CREATE USER john@localhost
```

if you want for all of your sub domain we do like this

```sql
CREATE USER john@'%.mysite.com'
```

and you can set password for user like this
